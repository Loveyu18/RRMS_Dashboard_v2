  ...
  if (pages.length <= 4) {
    pages.forEach((element, index) => {
      pageLi +=
        ...
        index +
        ",'.list_ofoperators_pages>div')\" id=\"page_" +
        index +
        '" class="page-item-link btn btn-icon btn-circle btn-sm border-0 btn-hover-success mr-2 my-1 ' +
    ...
    pageLi +=
      ...
      (0 + 1) +
      " </a>";
    pageLi += `<a id="pageDotL" class="page-item-link" style="font-size: 22px;cursor: default; display: ${
      currentPage <= 2 ? "none" : "block"
    }"><span>...</span></a>`;
    for (let i = 1; i < pages.length - 1; i++) {
      pageLi += `<a href="javascript:pageChange(${i},\'.list_ofoperators_pages>div\')" style="display:${
        i < currentPage + 2 && i > currentPage - 2 ? "inline-flex" : "none"
      }" id="page_${i}" class="page-item-link btn btn-icon btn-circle btn-sm border-0 btn-hover-success mr-2 my-1 '${
        currentPage === i ? "active" : ""
      }' " id="page_${i}"> ${i + 1} </a>`;
    }
    pageLi += `<a id="pageDotR" class="page-item-link" style="font-size: 22px;cursor: default; display: ${
      pages.length - 3 <= currentPage ? "none" : "block"
    }"><span>...</span></a>`;
    pageLi +=
      '<a href="javascript:pageChange(' +
      (pages.length - 1) +
...
function firstPage() {
  if (currentPage < pages.length && currentPage != 0) pageChange(0);
}
...
function endPage() {
  if (pages.length - 1 > currentPage) pageChange("#page_".length);
}
